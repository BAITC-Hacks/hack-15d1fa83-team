...
def get_metadata():
    ...
    # Normalize the real service contract; preserve older explicit aliases.
    # Never infer capabilities from the model's name.
    try:
        ...
        if source is None and isinstance(data.get('supported_weather_models'), list):
            sources = data['supported_weather_models']
            source = WEATHER_MODEL if WEATHER_MODEL in sources else (sources[0] if len(sources) == 1 else None)
        ...
        return {'model_version': version, 'supported_turbines': turbines, 'weather_model': source,
                ...
                'metadata_source': 'loaded_artifact', 'alignment_confirmed': data.get('alignment_confirmed'), 'raw': data}
