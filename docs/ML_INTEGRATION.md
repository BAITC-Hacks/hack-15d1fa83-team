...
## GET /v1/metadata — verified adapter boundary

Django reads fresh metadata before each inference/cache decision. The committed joint model reports both turbines. The actual service uses `supported_weather_models` and `fields`; the adapter normalizes these into `weather_model` and `field_definitions` for the dashboard while retaining raw metadata.

Relevant actual fields (the full response also includes schema versions, horizon, timezone, locations and archive semantics):
...
Legacy aliases remain supported; missing capability information fails visibly with 502. No default turbine support is invented. `GET /api/v1/ml/metadata/` returns normalized metadata. The upstream URL defaults to sibling `/v1/metadata` of `ML_SERVICE_URL`, with `ML_METADATA_URL` as an explicit override. Both the real artifact regression test and real live HTTP run verified this mapping.
...
Omit/unset the token if not configured upstream. `.env.example` is documentation; .env is not automatically loaded. Restart Django after changes. A GitHub branch is not a hosted API. The pre-final Compose stack supplies its internal URL automatically. Real-artifact end-to-end HTTP compatibility passed for both turbines; see [results](../reports/pre-final-live.json). Container build/run remains unverified.
